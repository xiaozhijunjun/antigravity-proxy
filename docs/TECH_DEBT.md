...
### ~~TD-008: IOCP 状态检查缺失~~ ✅ 已修复
- **文件**: `src/hooks/Hooks.cpp`
- **问题**: `GetQueuedCompletionStatusEx` 未检查 `OVERLAPPED_ENTRY.Internal`，失败事件仍握手
- **状态**: **已修复** (2026-01-23)
- **修复说明**: 失败事件直接清理上下文；成功事件才执行握手

### ~~TD-009: FakeIP 跨进程映射缺失~~ ✅ 已修复
- **文件**: `src/network/FakeIP.hpp`
- **问题**: 多进程场景下 FakeIP 映射仅存在于单进程，导致 `GetDomain` 未命中
- **状态**: **已修复** (2026-01-23)
- **修复说明**: 引入跨进程共享映射，未命中时回填本进程缓存
...
1. **短期**: TD-002, TD-003 (Map 清理)
2. **中期**: TD-004 (日志轮转)
3. **长期**: TD-005/TD-006/TD-007（稳态清理与兼容性）
